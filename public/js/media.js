...
function getHTMLMediaElement(mediaElement, config) {
    ...
    if (buttons.has('full-screen')) {
        ...
        zoom.onclick = function() {
            if (zoom.className.indexOf('zoom-out') != -1) {
              ...
              mediaBox.querySelector('video').classList.add('fullscreen');
                exitFullScreen();
            } else {
                zoom.className = zoom.className.replace('zoom-in', 'zoom-out selected');
                mediaBox.querySelector('video').classList.remove('fullscreen');
    ...
    if (config.title) {
        ...
        h2.setAttribute('style', 'position: absolute;color:white;font-size:17px;text-shadow: 1px 1px black;padding:0;margin:0;text-align: left; margin-top: 0px; margin-left: 2px; display: block; border: 0;line-height:1.5;z-index:1;');
